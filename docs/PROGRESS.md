...
| #32 | perf(edge) | Perf/CF | admin pages (8 + layout) + Server Actions (4) + api/admin (5) を edge runtime 化 (Phase 2c)。async factory pattern + next.config.mjs の `node:*` externals 追加 |
...
Phase 1 完了時点で `yarn build:cf` が依然 fail していたルートは **計 14** (admin layout を含めると 15):
- **admin pages**: 8 (`/admin`, `/admin/home`, `/admin/skill`, `/admin/profile`, `/admin/productions`, `/admin/productions/new`, `/admin/productions/[id]`, `/admin/login`) + `src/app/admin/layout.tsx`
...
| 2c | `perf/cf-edge-admin-pages` | ✅ merged (#32, 2026-06-21) | `src/app/admin/**` の 8 ページ + `layout.tsx` + Server Actions 4 + `api/admin/*` 5 を edge 化。next.config.mjs に edge layer 用の `node:fs` / `node:path` externals fallback を追加。 |
